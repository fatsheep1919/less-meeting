 ...
 */
export async function showLobby(container) {
...
// 模式一：主页 — 创建房间
// ============================================
function renderCreateOnly(container) {
    // 闭包中保存创建结果
    let createdRoomId = '';
    let createdPasscode = '';
    let createdRoomName = '';
  ...
  `;
    ...
    document.getElementById('btn-create-room').onclick = async () => {
        ...
        try {
            ...
            document.getElementById('input-room-url').value = urlStr;
    ...
    document.getElementById('btn-enter-meeting').onclick = () => {
        const url = document.getElementById('input-room-url').value;
        if (url)
...
// 模式二：直链 — 输入昵称加入
// ============================================
async function renderDirectLink(container, roomId, passcode) {
    // 获取房间名
    let roomName = '';
    try {
  ...
  `;
    ...
    document.getElementById('btn-join-room').onclick = () => {
        const displayName = document.getElementById('input-nickname').value.trim();
        if (!displayName) {
            showError('请输入你的昵称');
            return;
        }
...
function extractPasscodeFromQuery() {
    return new URLSearchParams(location.search).get('p');
}
async function joinRoom(roomId, passcode, displayName, roomName, showError) {
    try {
