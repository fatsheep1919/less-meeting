 ...
 * 会议页面
 */
export function showMeeting(container) {
    container.innerHTML = `
    <div class="meeting">
      <div id="room-header">
  ...
  `;
    ...
    // 正在说话的人
    const speakingPeerIds = new Set();
    // 刷新参会者网格
    function refreshGrid() {
        const grid = document.getElementById('peer-grid');
        if (grid)
            renderPeerGrid(grid, speakingPeerIds);
    }
    // ---- 绑定远程音频流 ----
    ...
    // ---- 说话状态 ----
    signaling.on('SPEAKER_ACTIVE', (msg) => {
        if (msg.type !== 'SPEAKER_ACTIVE')
            return;
        speakingPeerIds.add(msg.peerId);
        refreshGrid();
    });
    signaling.on('SPEAKER_INACTIVE', (msg) => {
